...
Generate.prototype.log = function log(format, path, limit, writeInterval) {
  const self = this;
...
Generate.prototype.line = function line(callbackCount, callbackLimit, interval) {
  ...
  const randomDate = new Date(faker.date.recent(10));
  ...
  if (self.callbackAccumulatorCount !== callbackLimit) {
    self.callbackAccumulatorCount += 1;
